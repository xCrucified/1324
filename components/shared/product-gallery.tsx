    ...
    <>
        ...
        >
          ...
          {imagesList.length > 1 && (
            <>
              <button
                onClick={handlePrev}
                className="absolute left-3 top-1/2 -translate-y-1/2 bg-ivory/90 hover:bg-ivory text-bark rounded-full w-9 h-9 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-md z-10 text-lg cursor-pointer"
              >
                ‹
              </button>
              <button
                onClick={handleNext}
                className="absolute right-3 top-1/2 -translate-y-1/2 bg-ivory/90 hover:bg-ivory text-bark rounded-full w-9 h-9 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-md z-10 text-lg cursor-pointer"
              >
                ›
              </button>
          ...
          <div className="flex gap-3 overflow-x-auto pb-1">
            {imagesList.map((img, idx) => (
              <button
                ...
                className={`relative w-28 h-20 rounded-sm overflow-hidden border-2 transition-all shrink-0 cursor-pointer ${
      ...
      {isOpenModal && (
        ...
        >
          ...
          {/* Центральна частина з великою картинкою та стрілками */}
          <div
            className="relative flex-1 w-full flex items-center justify-center px-6 md:px-14 py-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="relative w-full max-w-6xl h-[72vh] bg-white rounded-xl overflow-hidden shadow-[0_20px_80px_rgba(0,0,0,.45)] flex items-center justify-center">
              <img
                src={imagesList[currentIndex]}
                alt={title}
